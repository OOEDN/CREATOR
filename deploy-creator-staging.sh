#!/bin/bash
# ═══════════════════════════════════════════════════════
# OOEDN Creator Portal — STAGING Deploy Script
# Target: ooedn-creator-portal / creator-portal-staging
#
# This deploys to a SEPARATE Cloud Run service.
# Production (creator-portal) is NEVER touched.
...
echo ""
echo "🧪 ═══════════════════════════════════════════"
echo "   DEPLOYING CREATOR PORTAL (STAGING)"
echo "   Project: ooedn-creator-portal"
echo "   Service: creator-portal-staging"
...
gcloud run deploy creator-portal-staging --source . --region us-west1 \
  --update-env-vars "\
CREATOR_MODE=true,\
...
STAGING_URL=$(gcloud run services describe creator-portal-staging --region us-west1 --format='value(status.url)' 2>/dev/null)

echo ""
echo "✅ ═══════════════════════════════════════════"
echo "   STAGING CREATOR PORTAL DEPLOYED"
echo "   Dockerfile auto-restored by trap ↩️"
echo "   URL: $STAGING_URL"
echo "   Production is UNTOUCHED ✅"
echo ""
echo "   Test here first, then deploy prod with:"
echo "   bash deploy-creator.sh"
echo "═══════════════════════════════════════════════"
echo ""
